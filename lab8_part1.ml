...
Now, complete the funccccctor definition below.
......................................................................*)
...
module MakeInterval (Endpoint : ORDERED_TYPE) = 
  struct
    ...
    let intersect (intvl1 : interval) (intvl2 : interval) : interval =
      let min a b = if Endpoint.compare a b < 0 then a else b in
      let max a b = if Endpoint.compare a b > 0 then a else b in
      match intvl1, intvl2 with 
      | Empty, Empty
      | Interval (_, _), Empty
      | Empty, Interval (_, _) -> Empty
      | Interval (low1, high1), Interval (low2, high2) ->
        let lowest_high, highest_low = min high1 high2, max low2 low1 in
        if Endpoint.compare highest_low lowest_high > 0 then Empty
        else Interval (highest_low, lowest_high) ;; 
    end ;;
...
let intvl1 = IntInterval.create 30 60 ;;
let intvl2 = IntInterval.create 40 80 ;;
...
module MakeBestInterval 
    (Endpoint : ORDERED_TYPE) : (INTERVAL with type endpoint = Endpoint.t) =
  struct
    type endpoint = Endpoint.t 
    (* val create : endpoint -> endpoint -> interval
    val is_empty : interval -> bool
